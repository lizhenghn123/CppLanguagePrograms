...
struct ElemPosTree
{
	...
	enum
	{
		PA_SEGBITS = 16, PA_SEGMASK = 0xffff
	};
...
struct TokenPos
{
	...
	{
		while (m_pDocText[m_nNext] && !MCD_PSZCHR(szStopChars, m_pDocText[m_nNext])) m_nNext += MCD_CLEN(&m_pDocText[m_nNext]);
	}
...
struct ElemStack
{
	...
	void Unslot(TagPos& lp)
	{
		...
		if (n) pL[n].iSlotNext = p;
		if (p) pL[p].iSlotPrev = n;
...
struct SavedPos
{
	...
	enum
	{
		SPM_MAIN = 1, SPM_CHILD = 2, SPM_USED = 4, SPM_LAST = 8
	};
	int nSavedPosFlags;
};
