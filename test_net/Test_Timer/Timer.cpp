...
void Timer::wait()
{
    ...
    int64_t milliseconds = static_cast<int64_t>(Timestamp::timeDiffMs(now, when_));
...
{
     Timestamp now = Timestamp::now();
     return Timestamp::timeDiffMs(now, when_);
}

size_t Timer::cancel()
{
    timerQueue_->deleteTimer(this);
    return Timestamp::timeDiffMs(Timestamp::now(), when_);
}
